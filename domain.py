...
BUILD_SPEC: dict[str, Any] = {
  ...
  "requirement_text": "Organisation: Feuji Software Solution Pvt. Ltd.\nSegment: Healthcare\n\nAI SUGGESTED & EDITED PARAMETERS:\nTOOLS AND TECH STACKS: The application will utilize Python for backend processes, FastAPI for developing RESTful APIs, React for a dynamic and responsive front-end user interface, MongoDB as the database to store user interactions and medical data, and a Speech-to-Text API to enable voice-to-text interaction.\nKNOWLEDGE BASE SOURCES: Utilize reliable medical databases such as the Centers for Disease Control and Prevention (CDC) Database and PubMed for retrieving up-to-date and authoritative medical information.\nLLM FOUNDATION MODEL: The application will leverage the GPT-4 Health Beta model, which is specially tuned for healthcare applications to deliver accurate and contextually relevant medical advice.\nPERSONA AND TONE: The chatbot will maintain a compassionate and informative tone while being compliant with medical guidelines, ensuring it supports and reassures users effectively during interactions.\nSUPPORTED LANGUAGES: The application will facilitate communication in English, Hindi, and Spanish to cater to a diverse user base with varied linguistic backgrounds.\nMULTIMODAL CAPABI",
